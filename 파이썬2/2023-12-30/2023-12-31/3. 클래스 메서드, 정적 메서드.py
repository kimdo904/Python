...
# 인스턴스 변수는 인스턴스마다 값을 별도로 저장해야 하지만 클래스 변수는 하나의 값을 모든 인스턴스가 공유하기 때문에 메모리 공간의 낭비를 막을 수 있다.
...
print(man.country)  # 클래스 변수를 불러오는 것이 아니라 인스턴스 변수를 불러옴. man 객체의 country라는 인스턴스 변수와 country 클래스 변수 2개가 존재.
...
print('---------------------------')
print('객체를 이용해서 클래스 값 변경')
man.__class__.country = '영국'
print(Korean.country)

print('---------------------------')
print('클래스를 이용해서 클래스 값 변경')
...
print(hong.school)
hong.school = '경북고' # 클래스 변수가 변경된 게 아니라 hong이라는 인스턴스에 school 이라는 속성이 추가됨.
print(hong.school)
print(hong)
...
# 정적 메서드는 self와 cls를 모두 사용하지 않기 때문에 인스턴스 변수와 클래스 변수를 모두 사용하지 않는 메소드를 정의하는 경우에 적절

# 정적 메소드는 클래스에 소속이 되어 있지만, 인스턴스에는 영향을 주지 않고 또 인스턴스로부터 영향을 받지도 않는다.
