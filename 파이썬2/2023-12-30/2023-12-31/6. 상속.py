...
# 그래서 상속 관계에 있는 클래스를 표현할 때 부모 클래스와 자식 클래스라는 말을 사용

# 부모 클래스 : 상속해주는 클래스 / 슈퍼 클래스 super class, 기반 클래스 base class
...
# is ~a 관계는 '직원은 사람이다. employee is ~ a person' 또는 자동차는 차량이다.
# has ~a 관계는 상속이 아니라 구성 Cimposition으로 구현함.

# 객체 지향 프로그래밍을 처음 접하면 두 클래스가 어떤 관계를 갖고 있을 때, 반드시 상속 관계가 만들어져야한다고 생각하는 경우가 많다.
# is ~a 관계를 가질 경우에는 상속, has ~a 관계를 가질 경우에는 구성을 사용한다고 구분해서 생각한다면
# 어떤 경우에 상속을 활용해야 하는지 조금 더 쉽게 구분 가능하다.
...
# 그에 비해 서브 클래스 객체는 서브 클래스와 인스턴스 이면서 동시에 슈퍼 클래스의 인스턴스
