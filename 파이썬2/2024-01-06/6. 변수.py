...
# 맥os나 리눅스에서 루트 폴더는 "\"임.
# 윈도우, 맥os에서는 폴더이름이나 파일이름의 대소문자를 구분하지 않지만, 리눅스는 구분한다.

# 윈도우에서 폴더 구분은 "\" 백슬레시를 사용하고, 맥os나 리눅스에서는 "/" 슬레시를 사용함.
...
# 새로운 디렉토리 만들기
# os.makedirs() # os는 운영체제이고 makedirs는 새로운 디렉토리를 만들겠다는 뜻.
# os.makedirs() 함수로 새로운 디렉토리(폴더)를 만들 수 있다.
# os_path, idsir(path) : path의 디렉토리가 있는지 확인
