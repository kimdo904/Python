...
# pandas : 어디에 쓰이나? AI, 머신러닝(인공지능), 딥러닝, 그래프를 뽑아내기 위해
...
# 다음은 Series()를 이용해 형식의 데이터를 생성하는 방법
...
# 자동으로 데이터 라벨(0~4) 부여되고, 세로줄 라벨을 index라고 하고, 시퀀스 데이터는 values라고 한다.

# Series 데이터는 index와 value를 분리해서 가져올 수 있다.
# Series 데이터를 s라고 할 때 index는 s.index로, values는 s.values로 가져올 수 있다.
print(s1.index)
...
# dtype : object
# 변수 s2는 문자열과 숫자가 혼합이 되어 있어 타입이 object = 모든 것

# 데이터가 없는 경우, numpy를 임포트 한 후 np.nan으로 데이터가 없음을 표시할 수 있다.
import numpy as np
...
# 두 개의 DataFrame 데이터 df1와 df2는 길이가 같지 않음
# 길이가 같지 않더라도 연산이 가능
print(df1 + df2)
...
# 2012년부터 2016년 까지 우리나라 계절별 강수량(단위 mm)

table_data3 = {'봄' : [256.5,264.3,215.9,223.2,312.8],
               '여름' : [770.6,567.5,599.8,387.1,446.2],
               '가을' : [363.3,231.2,293.1,247.8,381.6],
...
# 판다스를 이용해 데이터 프레임에서 Age열의 평균을 구하시오.
