...
# 3) range(start, stop, step) / start부터 stop 사이의 정수 중에서 step만큼의 차이를 가지고 있다.
...
# len(seasons) : 지금은 4를 집어넣는 것과 같다.
for idx in range(len(seasons)):
    print(f'{seasons[idx]} / {seasons_eng[idx]}')

# sorted() : 전달된 반복가능 객체의 오름차순 정렬 결과를 반환
...
# zip() : 전달된 여러 개의 반복가능객체와 각 요소를 튜플로 묶어서 반환
